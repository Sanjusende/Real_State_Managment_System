...
export default function AgentProfilePage() {
  ...
  const [form, setForm] = useState({
    name: user?.name || '',
    phone: user?.phone || '',
    agencyName: user?.agencyName || 'EstateCraft Premier Partner',
  ...
  const handleAvatarFileChange = async (e) => {
    ...
    try {
      ...
      if (newAvatarUrl) {
        ...
        toast.success('Agent profile photo updated successfully!');
      }
    } catch (err) {
        ...
        <div className="bg-white rounded-3xl border border-slate-200/90 p-6 md:p-8 shadow-xs space-y-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 pb-6 border-b border-slate-100">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
              ...
              <div
                ...
                title="Click to upload agent photo"
              >
                {form.avatar ? (
              ...
              <div>
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1.5">
                  <span className="text-xl font-extrabold text-slate-950">{form.name}</span>
                ...
                >
                  ...
                  <span>{uploadingAvatar ? 'Uploading...' : 'Upload Agent Photo'}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormInput
              ...
              onChange={handleChange}
              placeholder="e.g. EstateCraft Premier Partner"
              required
            />
          </div>
